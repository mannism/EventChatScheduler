...
- **`components/chat/ChatInterface.tsx`**: Renders the conversation. It intelligently intercepts Markdown payloads generated by the LLM (specifically JSON code blocks marked as `schedule_download`) to dynamically spawn interactive React UI components (like `ViewScheduleButton`) instead of rendering raw text. Also implements an intelligent auto-scrolling system tailored to user pacing and animated typing indicators.
- **`components/StatusBadge.tsx`**: A dynamic client-side component that leverages the `navigator.onLine` API to provide real-time connection status feedback directly in the persistent glassmorphic header.
