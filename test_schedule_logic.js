...
function createSchedule(userProfile = defaultProfile) {
    ...
    validDates.forEach(day => {
        ...
        // Other sessions
        const others = daySessions.filter(s => 
            s.track !== "Supercharge" && !interested.includes(s)
        );

        // Mix interested and others, shuffle
        const mix = [...interested, ...others].sort(() => 0.5 - Math.random());
        
        // Available candidates
        let candidates = [...mandatory, ...mix];

        // Ensure we sort by start time for easier slotting
        candidates.sort((a,b) => new Date(a.startDateTime).getTime() - new Date(b.startDateTime).getTime());

        const daySchedule = [];
        
        let lunchScheduled = false;

        // Fixed networking slots
        ...
        const tryAddSession = (s, isLunch = false) => {
            ...
            for (const e of daySchedule) {
                ...
                // Allow 5 min gap: sStart >= eEnd + 5 mins OR sEnd + 5 mins <= eStart
                const gap = 5 * 60 * 1000;
                
                if (isLunch) {
                    // exact overlap check for lunch
                    if (isOverlap(sStart, sEnd, eStart, eEnd)) return false;
                } else {
        ...
        // Try to add a 1 hour lunch block between 11:30 and 14:00 (e.g. 11:30-12:30, 12:00-13:00, 12:30-13:30, 13:00-14:00)
        let lunchSlots = [
        ...
        schedule[day] = {
            sessions: daySchedule.map(s => ({
                ...
                presenters: s.presenters?.map(p => p.name).join(", ") || "",
